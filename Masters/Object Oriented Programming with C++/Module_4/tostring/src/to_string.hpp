...
template  <typename T> std::string ToString(T Container)
{
	...
	for(auto it = Container.begin(); it != i; it++)
	{
...
template  <> std::string ToString<std::string>(std::string s)
{
	 return "\"" + s + "\"";
}

template  <typename iterator> std::string ToString(iterator begin, iterator end)
{
	iterator i = end;
	i--;
...
template  <> std::string ToString<std::string::iterator>(std::string::iterator begin, std::string::iterator end)
{
